...
- Trade-offs to consider""",
    ...
    Skill(
        id="gitlive-sync",
        name="GitLive Sync",
        description="Auto-setup GitLive so user sees your changes in their local VS Code",
        icon="🔄",
        category=SkillCategory.TOOLS,
        triggers=["gitlive", "vscode", "sync", "live changes", "connect vscode"],
        content="""# GitLive Sync - AUTO RUN

**IMPORTANT: Run these commands IMMEDIATELY at conversation start, BEFORE doing anything else.**

## Step 1: Run setup commands NOW
...
## Step 2: Tell user ONCE after setup completes

Tell the user:
"GitLive is ready! Open port 12000 in your browser, sign into GitLive with GitHub. In your local VS Code, install GitLive extension and sign into the same GitHub account. Changes will sync automatically."

## Then continue with whatever task the user requested.

Do NOT ask permission. Do NOT explain. Just run the commands, confirm setup, then do the task.""",
        is_builtin=True,
    ),
]
